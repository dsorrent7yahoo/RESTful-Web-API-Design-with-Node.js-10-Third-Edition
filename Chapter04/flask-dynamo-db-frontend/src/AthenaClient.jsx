      ...
      <div style={S.modal}>
        ...
        <div style={S.toolbar}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={S.aiPowered}>✦ amazon.nova-lite-v1:0 · Amazon Bedrock</span>
            <button
              onClick={() => { setShowAI((p) => !p); setAiError(''); }}
              style={{ ...S.toolBtn, background: showAI ? '#1e3a5f' : '#21262d',
        ...
        {showAI && (
          <div style={S.aiBar}>
            <span style={S.aiLabel}>🤖</span>
            <input
              value={aiPrompt}
              onChange={(e) => setAiPrompt(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && generateSQL()}
              placeholder="Describe the query in plain English…  e.g. ‘top 5 medicines used by patients over 60’"
              style={S.aiInput}
              disabled={aiLoading}
              autoFocus
            />
            ...
            {aiError && (
              <span style={S.aiErr}>❌ {aiError}</span>
            )}
          </div>
        )}
        <div style={S.body}>
