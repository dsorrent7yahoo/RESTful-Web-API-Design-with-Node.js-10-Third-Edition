          ...
          {docsMode !== 'readme' && docsMode !== 'deploy' && (
            <div style={{ width: '230px', flexShrink: 0, overflowY: 'auto',
              ...
              {docsMode === 'lambdas' && (
                <>
                  <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase',
                    letterSpacing: '0.1em', color: '#7c3aed', marginBottom: '8px' }}>Lambda Functions</div>
                  {LAMBDA_FILES.map(f => (
                    <div key={f.path} onClick={() => loadSourceFile(f.path)}
                      style={{ padding: '5px 8px', borderRadius: '5px', cursor: 'pointer', marginBottom: '3px',
                        fontFamily: 'monospace', fontSize: '12px',
                        background: srcSelectedPath === f.path ? '#f5f3ff' : 'transparent',
                        color: srcSelectedPath === f.path ? '#5b21b6' : '#374151',
                        border: srcSelectedPath === f.path ? '1px solid #c4b5fd' : '1px solid transparent' }}>
                      {f.label}
                    </div>
                  ))}
                </>
