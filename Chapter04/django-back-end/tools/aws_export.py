...
class DynamoToS3Exporter:
    ...
    def download_object(self, bucket, key):
        ...
        Returns:
            {'bucket': bucket, 'key': key, 'filename': str,
             'contentType': str, 'sizeBytes': int, 'body': str}
        """
        ...
        filename = key.split('/')[-1] if key else 'download'
        return {
            'bucket': bucket,
            'key': key,
            'filename': filename,
    ...
    def list_dynamo_tables(self):
        """Return a list of all DynamoDB table names in the account."""
        tables = []
        paginator = self._dynamo_client.get_paginator('list_tables')
    ...
    def export_all_to_data_lake(
        self,
        bucket,
        glue_database='healthcare_data_lake',
        prefix='datalake/',
        fmt='csv',
        create_bucket=False,
        create_database=True,
        ...
        """
        Export every DynamoDB table to S3 and register each in Glue.

        Returns:
            {'tables': [{tableName, s3Uri, rows, sizeBytes, glueAction}, ...],
             'count': int, 'bucket': bucket, 'glueDatabase': glue_database}
        """
        if not bucket:
            raise ValueError('bucket is required')
        if create_bucket:
            self.create_bucket_if_missing(bucket)
        tables = self.list_dynamo_tables()
        results = []
        for table_name in tables:
            try:
                export_result = self.export_table_to_s3(
                    table_name=table_name,
                    bucket=bucket,
                    prefix=f'{prefix.rstrip("/")}/{table_name}/',
                    fmt=fmt,
                    create_bucket=False,
                )
                glue_result = self.register_glue_table(
                    database=glue_database,
                    table_name=table_name,
                    s3_uri=f's3://{bucket}/{prefix.rstrip("/")}/{table_name}/',
                    fmt=fmt,
                    create_database=create_database,
                )
                results.append({
                    'tableName': table_name,
                    's3Uri': export_result['s3Uri'],
                    'rows': export_result['rows'],
                    'sizeBytes': export_result['sizeBytes'],
                    'glueAction': glue_result['action'],
                })
            except Exception as exc:
                results.append({'tableName': table_name, 'error': str(exc)})
        return {
            'tables': results,
            'count': len(results),
            'bucket': bucket,
    ...
    def export_csv_to_data_lake(
        ...
        """
        Upload local CSV files from csv_dir to S3 and register each in Glue.

        Returns:
            {'files': [{fileName, s3Uri, sizeBytes, glueAction}, ...],
             'count': int, 'bucket': bucket, 'glueDatabase': glue_database}
        """
        ...
        if not bucket:
            raise ValueError('bucket is required')
        if not csv_dir:
            raise ValueError('csvDir is required')
        if create_bucket:
            self.create_bucket_if_missing(bucket)
        csv_files = _glob.glob(os.path.join(csv_dir, '*.csv'))
        results = []
        for local_path in csv_files:
            table_name = os.path.splitext(os.path.basename(local_path))[0]
            s3_key = f'{prefix.rstrip("/")}/{table_name}/{table_name}.csv'
            try:
                upload_result = self.upload_local_csv_to_s3(local_path, bucket, s3_key)
                glue_result = self.register_glue_table(
                    database=glue_database,
                    table_name=table_name,
                    s3_uri=f's3://{bucket}/{prefix.rstrip("/")}/{table_name}/',
                ...
                results.append({
                    ...
                    'sizeBytes': upload_result['sizeBytes'],
                    'glueAction': glue_result['action'],
                })
            except Exception as exc:
                results.append({'fileName': os.path.basename(local_path), 'error': str(exc)})
        return {
            'files': results,
            'count': len(results),
            'bucket': bucket,
...
# Alias used by export_app
AwsExporter = DynamoToS3Exporter