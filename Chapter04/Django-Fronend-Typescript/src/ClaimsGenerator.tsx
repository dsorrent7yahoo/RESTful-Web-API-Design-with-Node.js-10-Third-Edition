...
export default function ClaimsGenerator({
  show, onClose, baseUrl, cleanerUrl, authToken,
  catalogEnabled = true, generateOnOpen = false, onGenerateSuccess,
}: ClaimsGeneratorProps) {
  const [files,           setFiles]           = useState<ClaimsFile[]>([]);
  const [loadingFiles,    setLoadingFiles]    = useState(false);
  const [viewKey,         setViewKey]         = useState<string | null>(null);
  const [viewRows,        setViewRows]        = useState<Record<string, unknown>[] | null>(null);
  const [viewCols,        setViewCols]        = useState<string[]>([]);
  ...
  const [processAllResult, setProcessAllResult] = useState<ProcessAllResult | null>(null);
  const [generating,      setGenerating]      = useState(false);
  const [generateResult,  setGenerateResult]  = useState<GenerateResult | null>(null);

  const authHeaders = { Authorization: `Bearer ${authToken}` };
  const _cleanerUrl = cleanerUrl ?? baseUrl;
  ...
  useEffect(() => {
    ...
    if (generateOnOpen) {
      ...
      fetch(`${baseUrl}/claims/generate`, { method: 'POST', headers: authHeaders })
        .then((r) => r.json())
        .then((data: GenerateResult) => { setGenerateResult(data); if (onGenerateSuccess) onGenerateSuccess(); })
  ...
  const handleCleanFile = async (file: ClaimsFile) => {
    setCleaningKey(file.key);
    setError('');
    try {
      const resp = await fetch(`${_cleanerUrl}/claims/clean`, {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: file.key, delete_after: true }),
      });
      const data: CleanResult = await resp.json();
  ...
  const handleProcessAll = async () => {
    ...
    setError('');
    try {
      const resp = await fetch(`${_cleanerUrl}/claims/process-all`, {
        method: 'POST',
        headers: { ...authHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({ delete_after: true }),
      });
      const data: ProcessAllResult = await resp.json();
  ...
  const handleView = async (file: ClaimsFile) => {
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px',
          ...
          {generateResult && !generating && (
            <div style={{ background: generateResult.statusCode === 200 ? '#f0fdf4' : '#fef2f2',
              ...
              {generateResult.statusCode === 200
                ? <>✅ Generated <code>{String(generateResult.key ?? '').split('/').pop()}</code> — {generateResult.rows_written} rows</>
                : <>❌ {generateResult.message ?? 'Generate failed'}</>}
            </div>
          )}

          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '13px', color: '#64748b' }}>s3://dgs-glue-staging/claims/</span>
            <button onClick={fetchFiles} disabled={loadingFiles}
              style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '6px',
                padding: '6px 14px', fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>
              {loadingFiles ? 'Loading...' : '🔄 Refresh'}
            </button>
            {catalogEnabled && files.length > 0 && (
              <button onClick={handleProcessAll} disabled={processingAll}
                style={{ background: 'linear-gradient(135deg,#065f46,#0e7490)', color: '#fff', border: 'none',
                  borderRadius: '6px', padding: '6px 14px', fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>
                {processingAll ? '⏳ Processing...' : '🗄️ Process All → Glue'}
              </button>
            )}
          </div>
          ...
          {processAllResult && (
            <pre style={{ background: '#0f172a', color: '#e2e8f0', borderRadius: '8px',
              padding: '12px', fontSize: '12px', overflow: 'auto', maxHeight: '200px' }}>
              {JSON.stringify(processAllResult, null, 2)}
            </pre>
          )}
          ...
          {files.map((f) => (
            <div key={f.key}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px',
                ...
                <button onClick={() => handleView(f)}
                  style={{ background: viewKey === f.key ? '#0ea5e9' : '#e0f2fe',
                    color: viewKey === f.key ? '#fff' : '#0369a1', border: 'none',
                    borderRadius: '5px', padding: '4px 12px', fontWeight: 600, fontSize: '12px', cursor: 'pointer' }}>
                  {viewKey === f.key ? 'Hide' : 'View'}
                </button>
                {catalogEnabled && (
                  <button onClick={() => handleCleanFile(f)}
                    disabled={cleaningKey === f.key || cleanResults[f.key]?.statusCode === 200}
                    style={{
                      background: cleanResults[f.key]?.statusCode === 200 ? '#e5e7eb'
                        : cleaningKey === f.key ? '#d1fae5'
                        : 'linear-gradient(135deg,#065f46,#0e7490)',
                      color: cleanResults[f.key]?.statusCode === 200 ? '#9ca3af'
                        : cleaningKey === f.key ? '#065f46' : '#fff',
                      border: cleanResults[f.key]?.statusCode === 200 ? '1px solid #d1d5db' : 'none',
                      borderRadius: '5px', padding: '4px 12px', fontWeight: 600, fontSize: '12px',
                      cursor: (cleaningKey === f.key || cleanResults[f.key]?.statusCode === 200) ? 'not-allowed' : 'pointer',
                      whiteSpace: 'nowrap',
                    }}>
                    {cleaningKey === f.key ? '⏳ Cataloging...' : cleanResults[f.key]?.statusCode === 200 ? '✅ Cataloged' : '🗄️ Clean & Catalog'}
                  </button>
                )}
              </div>
              ...
              {viewKey === f.key && (
                <div style={{ marginBottom: '8px', border: '1px solid #bae6fd', borderRadius: '6px', overflow: 'hidden' }}>
                  {loadingRows && <p style={{ padding: '16px', fontSize: '13px', color: '#64748b' }}>Loading…</p>}
                    ...
                    <div style={{ overflow: 'auto', maxHeight: '400px' }}>
                      <table style={{ borderCollapse: 'collapse', fontSize: '12px', width: 'max-content', minWidth: '100%' }}>
                        <thead>
                          <tr>
                            {viewCols.map((c) => (
                              <th key={c} style={{ position: 'sticky', top: 0, zIndex: 2, background: '#0ea5e9', color: '#fff',
                                padding: '7px 10px', textAlign: 'left', whiteSpace: 'nowrap', fontWeight: 600,
                          ...
                          {viewRows.map((row, ri) => (
                            <tr key={ri} style={{ background: ri % 2 === 0 ? '#fff' : '#f0f9ff' }}>
                              {viewCols.map((c) => {
                                ...
                                return (
                                  <td key={c} style={{ padding: '5px 10px', borderBottom: '1px solid #e2e8f0',
                                    borderRight: '1px solid #f1f5f9', whiteSpace: 'nowrap',
                                    color: missing ? '#dc2626' : 'inherit', fontWeight: missing ? 700 : 'normal' }}>
                                    {missing ? '⚠ missing' : String(row[c] ?? '')}
                                  </td>
                                );
                              })}
                            </tr>
