...
interface PatientsEncountersProps {
  show: boolean;
  onClose: () => void;
  baseUrl: string;
  authToken: string;
...
const downloadCSV = (rows: Record<string, unknown>[], filename = 'data.csv') => {
...
export default function PatientsEncounters({ show, onClose, baseUrl, authToken }: PatientsEncountersProps) {
  const [activeTab, setActiveTab]           = useState('patients');
  const [loading, setLoading]               = useState(false);
  const [result, setResult]                 = useState<GenerateResult | null>(null);
  const [error, setError]                   = useState('');
  const [total, setTotal]                   = useState(100);
  const [patientFiles, setPatientFiles]     = useState<S3File[]>([]);
  const [encounterFiles, setEncounterFiles] = useState<S3File[]>([]);
  const [filesLoading, setFilesLoading]     = useState(false);

  const api = useCallback(async (path: string, opts: RequestInit = {}) => {
    const url  = `${(baseUrl ?? 'http://localhost:4015').replace(/\/$/, '')}${path}`;
    const resp = await fetch(url, {
      headers: { Authorization: `Bearer ${authToken}`, 'Content-Type': 'application/json', ...(opts.headers as Record<string, string> | undefined) },
      ...opts,
    });
    const data = await resp.json().catch(() => ({}));
    if (!resp.ok) throw new Error((data as { detail?: string; error?: string }).detail ?? (data as { error?: string }).error ?? `HTTP ${resp.status}`);
    return data;
  }, [baseUrl, authToken]);

  const generate = useCallback(async (resource: string) => {
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      ...
      setPatientFiles((pf as { files?: S3File[] }).files ?? []);
      setEncounterFiles((ef as { files?: S3File[] }).files ?? []);
    } catch (e) {
      setError((e as Error).message);
    } finally {
  ...
  const btnStyle = (color = '#7c3aed'): JSX.CSSProperties => ({
    background: color, color: '#fff', border: 'none', borderRadius: '6px',
    padding: '6px 16px', fontWeight: 700, fontSize: '13px',
    cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1,
  });

  const renderGenerate = (resource: string, label: string) => (
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
        ...
        <input
          ...
          onChange={(e) => setTotal(Math.max(1, parseInt(e.currentTarget.value) || 100))}
      ...
      {error && (
        <div style={{ background: '#450a0a', border: '1px solid #b91c1c', borderRadius: '6px',
  ...
  const renderFiles = () => (
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <button onClick={loadFiles} disabled={filesLoading || !authToken} style={btnStyle('#0f766e')}>
      ...
      ] as const).map(({ label, files, prefix }) => (
        <div key={prefix}>
