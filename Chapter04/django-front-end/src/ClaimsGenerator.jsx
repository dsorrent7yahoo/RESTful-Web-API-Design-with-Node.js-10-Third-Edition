...
export default function ClaimsGenerator({ show, onClose, baseUrl, cleanerUrl, authToken, catalogEnabled = true, generateOnOpen = false }) {
  ...
  // cleanerUrl defaults to baseUrl when not in microservices mode
            ...
            <div key={f.key}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px',
                ...
                <span style={{ flex: 1, fontWeight: 600, fontSize: '13px' }}>
                  &#x1F4C4; {f.name || (f.key ? f.key.split('/').pop() : f.key)}
                </span>
                ...
                {catalogEnabled && (
                  <button onClick={() => handleCleanFile(f)}
                    ...
                    style={{
                      background: cleanResults[f.key]?.statusCode === 200
                        ? '#e5e7eb'
                        : cleaningKey === f.key
                          ? '#d1fae5'
                          : 'linear-gradient(135deg,#065f46,#0e7490)',
                      color: cleanResults[f.key]?.statusCode === 200
                        ? '#9ca3af'
                      ...
                      whiteSpace: 'nowrap',
                      opacity: cleanResults[f.key]?.statusCode === 200 ? 0.6 : 1,
                    }}>
