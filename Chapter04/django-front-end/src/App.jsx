...
const BACKEND_PRESETS = {
  cmdline: 'http://localhost:4002',   // python manage.py runserver 4002
  docker:  'http://localhost:4002',   // docker compose
  aws:     `${GATEWAY_ORIGIN}/proxy/django`,
};

// Microservice URLs — localhost for dev, gateway proxy for EC2/Fargate
const MICROSERVICE_URLS = IS_FARGATE ? {
...
export default function App() {
  ...
  const [serviceUrls, setServiceUrls]           = useState({ ...MICROSERVICE_URLS });
        ...
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px',
          ...
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <p style={{ fontSize: '13px', color: '#94a3b8', margin: 0 }}>
              {IS_FARGATE ? 'AWS Fargate' : 'localhost:4002'} — sign in to continue
            </p>
          </div>
  ...
  return (
    <main className="page">
        ...
        <div>
          ...
          <div style={{ marginTop: '16px' }}>
            ...
            <div style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button type="button"
                onClick={() => setMicroservicesMode(m => !m)}
                style={{
              ...
              {microservicesMode && (
                <span style={{ fontSize: '11px', color: '#0f766e' }}>
                  UI widgets talk directly to individual services on ports 30000–30005
                </span>
              )}
            </div>

            {/* Per-service URL editor — always visible */}
            <div style={{ marginTop: '10px', background: '#0f172a', border: '1px solid #1e3a5f',
                borderRadius: '8px', padding: '12px', fontSize: '12px' }}>
                <div style={{ fontWeight: 700, color: '#7dd3fc', marginBottom: '8px', letterSpacing: '0.08em' }}>
                  MICROSERVICE URLS
                </div>
                {[
                  ...
                  <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                    ...
                    <input
                      value={serviceUrls[key] || `http://localhost:${port}`}
                      onChange={e => setServiceUrls(u => ({ ...u, [key]: e.target.value }))}
          ...
          <div style={{ marginTop: '10px', fontSize: '13px', display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button type="button"
              ...
              style={{ background: 'linear-gradient(135deg,#7e22ce,#6d28d9)', color: '#e9d5ff', border: '1px solid #8b5cf6',
                borderRadius: '6px', padding: '5px 14px', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>
              🧬 Patients &amp; Encounters
            </button>
          </div>
        ...
        <div className="modal-backdrop" onClick={closeLoaderModal}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="meta" style={{ marginTop: '8px' }}>
              ...
              The Django backend creates the table automatically if it does not exist.
            </div>
      ...
      <ClaimsGenerator
        ...
        cleanerUrl={microservicesMode ? serviceUrls.claimsCleaner : (microservicesMode ? serviceUrls.claimsGenerator : baseUrl)}
        authToken={authToken}
      ...
      <SQSMonitor
        ...
        baseUrl={microservicesMode ? serviceUrls.sqsMonitor : baseUrl}
        authToken={authToken}
      />
      ...
      <AthenaClient
        ...
        baseUrl={microservicesMode ? serviceUrls.athenaClient : baseUrl}
        authToken={authToken}
      />

      {/* Patients & Encounters Generator */}
      <PatientsEncounters
        show={showPatientsModal} onClose={() => setShowPatientsModal(false)}
        baseUrl={microservicesMode ? serviceUrls.patientsEncounters : baseUrl}
        authToken={authToken}
      />
      ...
      <section className="panel">
        ...
        <p style={{ fontSize: '13px', color: '#475569', marginBottom: '12px' }}>
          Browse the full project source — Django backend, React UI, Terraform infrastructure, Docker config, Jupyter notebooks, and GitHub Actions CI/CD pipelines.
        </p>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap' }}>
          ...
          <select value={srcQuickFile} onChange={(e) => setSrcQuickFile(e.target.value)}
            ...
            <optgroup label="Django App">
              <option value="django-back-end/manage.py">manage.py  (Django management commands)</option>
              <option value="django-back-end/Dockerfile">Dockerfile  (container build)</option>
              <option value="django-back-end/requirements.txt">requirements.txt</option>
              <option value="django-back-end/openapi.json">openapi.json  (API spec)</option>
            </optgroup>
            <optgroup label="Django Routes">
              <option value="django-back-end/medications/views.py">medications/views.py  (CRUD)</option>
              <option value="django-back-end/auth_app/views.py">auth_app/views.py  (JWT auth)</option>
              <option value="django-back-end/export_app/views.py">export_app/views.py  (S3 export + Glue)</option>
              <option value="django-back-end/source_app/views.py">source_app/views.py  (source browser API)</option>
            </optgroup>
            <optgroup label="Kubernetes">
              <option value="django-back-end/kubernetes-workflow.md">☸️ kubernetes-workflow.md  (K8s workflow &amp; pod lifecycle)</option>
            </optgroup>
