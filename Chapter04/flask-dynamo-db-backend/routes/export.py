...
# ---------------------------------------------------------------------------
# CSV file upload → S3 → Glue (browser multipart upload)
# ---------------------------------------------------------------------------

@export_bp.post("/upload/glue-csv")
@jwt_required
def upload_csvs_to_glue():
    """
    Accept one or more CSV files uploaded from the browser, copy each to S3,
    and register each in the Glue Data Catalog.

    multipart/form-data fields:
        files[]          one or more CSV files (required)
        database         Glue database name   (default: healthcare_data_lake)
        bucket           S3 bucket            (required)
        prefix           S3 key prefix root   (default: datalake/)
        createDatabase   true|false           (default: true)
    ...
    Returns JSON:
        {
            ...
            "results": [
                {"file": "...", "table": "...", "s3Uri": "...",
                 "columns": 5, "action": "created|updated", "status": "ok"},
                ...
            ]
        }
    """
